...
import edu.rutgers.css.Rutgers.ui.MainActivityBus;

/**
 * Perform the initial load and post to an event bus
 */
public class InitLoadAsyncTask extends AsyncTask<InitLoadArgs, Void, Bundle> {
    private final String homeCampus;
    private final String defaultCampusCode;
    private final String defaultLevelCode;
    private final String defaultSemesterCode;

    public InitLoadAsyncTask(final String homeCampus,
    ...
    @Override
    protected Bundle doInBackground(InitLoadArgs... args) {
        if (args.length != 1) {
            return null;
        }

        InitLoadArgs firstArgs = args[0];
    ...
    protected void onPostExecute(Bundle args) {
        if (args != null) {
            MainActivityBus.getInstance().post(args);
        }
    }
    ...
    private Bundle switchSOC(List<String> pathParts) {
        ...
        if (courseCode != null && subjectCode != null) {
            ...
            return SOCSections.createArgs("ayy", semesterCode, campusCode, subjectCode, courseCode);
        }
        ...
        if (subjectCode != null) {
            ...
            return SOCCourses.createArgs("ayy", campusCode, semesterCode, levelCode, subjectCode);
        }
